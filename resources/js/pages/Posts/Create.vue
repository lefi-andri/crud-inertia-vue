...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="w-full mx-auto p-6">
          ...
          <form @submit.prevent="form.post('/posts')" class="space-y-5">
            <div>
              <label class="block text-sm font-medium mb-1">Title</label>
              <input v-model="form.title" type="text" class="w-full rounded-2xl border p-2 border-slate-300 focus:border-slate-400 focus:ring-0" />
            ...
            <div class="flex items-center gap-3">
              <button :disabled="form.processing" type="submit" class="px-4 py-2 rounded-2xl bg-slate-900 text-white hover:opacity-90 disabled:opacity-60">Save</button>
